...
// Function to log in to MagicChat
async function loginToMagicChat(page, email, password) {
    console.log("Opening MagicChat...");
    await page.goto(MAGICCHAT_SIGNIN_URL, { waitUntil: "networkidle2" });
    // Wait for login fields
    await page.waitForSelector("input[type='email']", { timeout: 60000 });
...
async function navigateToBotsPage(page) {
...
async function createMagicChatBot(botName, email, password) {
...
server.setRequestHandler(CallToolRequestSchema, async (request) => {
    if (request.params.name === "create_bot") {
        const { bot_name, email, password } = request.params.arguments;
        try {
